...
\textbf{Notes:} ACM Digital Library did not support a mix of ANDs and ORs in its initial input field, but this was possible in advanced search. The search string was not modified, and the first search gave us a satisfactory number of results. 
\newline
\newline
\textbf{Queries:}
\begin{itemize}
...
\textbf{Notes:} In ScienceDirect it was only possible to perform a full text search. The first search we conducted was within ``all sciences'' and this gave us 100 results. We therefore decided to just search within ``Computer Science'', which gave us less, but a lot more relevant papers. In addition to this we chose to exclude books, and just search within articles. 
\newline
\newline
\textbf{Queries:}
\begin{itemize}
...
\section{CiteSeer}
\textbf{Notes:} In CiteSeer you cannot perform a search within title, abstract and keywords at the same time. We therefore conducted a full text search by adding the element text:() to the query.  Some of the retrieved papers had an unknown title with unknown authors, so we chose to exclude these papers. The complete search string gave us 27 results, so we settled with this result. 
\newline
\newline
\textbf{Queries:}
\begin{itemize}
...
\textbf{Notes:} In SpringerLinks advanced search it was only possible to find resources with either all the words, the exact phrase or at least one of the words. We therefore used the whole boolean search string in the initial input field. The first search gave us 200 results, so we chose to only search within ``computer science'' and ``engineering''. In addition to this we removed ``book chapters'' from the search and only searched within articles. 
\newline
\newline
\textbf{Queries:}
\begin{itemize}
