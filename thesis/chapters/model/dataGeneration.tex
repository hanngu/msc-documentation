...
\citet{mumford13}, which consist of text files:
...
\textit{The MandlCoord.txt file includes 15 lines, with the (x,y) coordinates of the 15 nodes. These coordinates was not supplied in Mandl's literature, so these are copied from \citet{mumford13} and are approximate for the picture to be drawn.}
\end{table}
...
\begin{figure}[h!]
  ...
  \caption[Transit Network]
   ...
   \textit{The transit network is including the 15 nodes and 21 edges. The graph is undirected, because most lines use same arcs in both directions. Coordinates are correct based on the mandlCoord.txt file from \citep{mumford13}}
\end{figure}
