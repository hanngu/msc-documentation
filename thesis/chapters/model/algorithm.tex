...
\emph{\color{red} The selection of which algorithm is used is described under.}
...
As mentioned does Neo4j[] includes two built-in algorithms for finding the shortest path, including A* search\citep{russel10}  and Dijkstra's\citep{cormen09}. The most widely known form of best-first search is called A* search\cite[p.93-94]{russel10}. It evaluates nodes by combining(n), the cost to reach the node, and h(n), the cost to get from the node to the goal.
$$ f(n) = g(n) + h(n).$$
