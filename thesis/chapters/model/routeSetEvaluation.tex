...
After each ant in the Combined Swarm colony has created a complete route set, the ants are evaluated. The route sets are evaluated as a whole, because the connectivity and the paths chosen are dependent of the entire route set. The results of the evaluation determines which ants to be followed in the next iteration and whether or not the global best solution so far is updated. 
