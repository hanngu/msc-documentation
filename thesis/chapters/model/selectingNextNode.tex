...
If ant $a$ is a $FA$, $a$ chooses the edges chosen by the ant it is following. Described in Section \vref{sec:selctingAntsToBeFollowed}, a percentage of the best ants are added to a list, $ba$, where $a_1$ follows $ba_1$, $a_2$ follows $ba_2$, and so on. %The percentage of $FA$ is a predefined parameter, selected after excessive testing.