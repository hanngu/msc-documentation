...
For the reasons stated above, we will in this thesis focus on the UTRP, creating effective bus routes on an existing urban transit network, with an ACO algorithm inspired by both BCO and PSO. This will help us establish Research Question \vref{itm:2a}, which is concerned about whether or not it is efficient to combine attributes from different swarm intelligence methods in order to improve the computational results. We will compare the results with performance criteria used in similar research, and determine the effectiveness of the algorithm. 
